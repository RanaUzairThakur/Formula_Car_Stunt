...
public class RCC_WheelCollider : RCC_Core {
	...
	[Range(-1f, 500f)]public float powerMultiplier = 4f;
	public bool canSteer = false;		//	Can this wheel steer?
	[Range(-1f, 500f)]public float steeringMultiplier = 1f;
	...
	public void ApplyBrakeTorque(float torque){
		...
		if (Mathf.Abs(torque) > 1f)
		{
		//	print("BrakeTorque :" + torque);
			wheelCollider.brakeTorque = torque;
		//	wheelCollider.motorTorque -= Mathf.Clamp(torque * (wheelHit.forwardSlip) * carController.TCSStrength, 0f, Mathf.Infinity);
		}
		else
