...
public class AdmobAdsManager : MediationHandler
{
    ...
    void Start()
    {
        ...
        if (Application.internetReachability != NetworkReachability.NotReachable && (PlayerPrefs.GetInt("RemoveAds") != 1))
        {
            ...
            if (Toolbox.DB.Prefs.Is_DeviceConditionBad())
            {
    ...
    public void ShowRewardedMonster()
    {
        if (mediation != null && Application.internetReachability != NetworkReachability.NotReachable)
        {
            //  if (mediation.IsRewardedAdReady())
            //   {
            mediation.ShowRewardedVideo(GiveMonster);
            //   }
            //    else
            //    {

            LoadRewardedVideo();
            //     levelmanager.instance.novideopopup();
            //  }
        }
    }
    ...
    void GiveMonster()
    {
        ...
        //}
        Debug.Log("User Rewarded Monster");
    }

    public void ShowRewardedToy()
    {
        if (mediation != null && Application.internetReachability != NetworkReachability.NotReachable)
        {
            //  if (mediation.IsRewardedAdReady())
            //  {
            mediation.ShowRewardedVideo(GiveToy);
            //  }
            //   else
            //    {
            LoadRewardedVideo();
    ...
    void GiveToy()
    {
        ...
        //}
        Debug.Log("User Rewarded Toy");
    }
    public void ShowRewardedToy2()
    {
        if (mediation != null && Application.internetReachability != NetworkReachability.NotReachable)
        {
            //  if (mediation.IsRewardedAdReady())
            //  {
            mediation.ShowRewardedVideo(GiveToy2);
            //   }
            //   else
            //    {
            LoadRewardedVideo();
    ...
    void GiveToy2()
    {
        ...
        //    levelmanager.instance.mommyon();
        //    levelmanager.instance.RobotoTiger();
        //}
        Debug.Log("User Rewarded Toy");
    }
    ...
    {
        if (IsSmallBannerReady() || smallBannerStatus == AdLoadingStatus.Loading || adsStatus == AdType.NOAds)
        {
            return;
        }
    ...
    {
        if (IsMediumBannerReady() || mediumBannerStatus == AdLoadingStatus.Loading || adsStatus == AdType.NOAds)
        {
            return;
        }
        if (Application.internetReachability == NetworkReachability.ReachableViaCarrierDataNetwork | Application.internetReachability == NetworkReachability.ReachableViaLocalAreaNetwork)
    ...
    {
        if (!isAdmobInitialized || IsInterstitialAdReady() || iAdStatus == AdLoadingStatus.Loading || Constantss.adsRemoteConfigStatus == "0" || adsStatus == AdType.NOAds)
        {
            return;
        }
        if (Application.internetReachability == NetworkReachability.ReachableViaCarrierDataNetwork | Application.internetReachability == NetworkReachability.ReachableViaLocalAreaNetwork)
    ...
    //{
    //    if (!isAdmobInitialized || IsVideoAdReady() || vAdStatus == AdLoadingStatus.Loading || Constantss.adsRemoteConfigStatus == "0" || adsStatus == AdType.NOAds)
    //    {
    //        return;
    //    }
    //    if (Application.internetReachability == NetworkReachability.ReachableViaCarrierDataNetwork | Application.internetReachability == NetworkReachability.ReachableViaLocalAreaNetwork)
    ...
    public override void ShowSmallBanner(AdPosition position)
    {
        ...
        try
        {
            if (!isAdmobInitialized || !isAdmobBannerInitialized)
            {
                return;
            }
            if (SmallBanner_High_Ecpm)
    ...
    //public  void ShowSmallSmartBanner(AdPosition position)
    //{
