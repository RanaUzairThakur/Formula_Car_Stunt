...
{
    private GameObject Bostshashka, Carshadow;
    private bool showshadow = false;

    // Getting an Instance of Main Shared RCC Settings.
    #region RCC Settings InstanceF
    ...
    public bool AIController { get { return externalController; } set { externalController = value; } }
    ...
    void Start()
    {
        Carshadow = GameObject.FindWithTag("shadow");
    ...
    private void Awake()
    {
        ...
        switch (RCCSettings.behaviorType)
        {
            ...
            case RCC_Settings.BehaviorType.Fun:
                ...
                steerHelperLinearVelStrength = Mathf.Clamp(steerHelperLinearVelStrength, .5f, 1f);
                steerHelperAngularVelStrength = Mathf.Clamp(steerHelperAngularVelStrength, 1f, 1f);
                highspeedsteerAngle = Mathf.Clamp(highspeedsteerAngle, 30f, 35f);
                highspeedsteerAngleAtspeed = Mathf.Clamp(highspeedsteerAngleAtspeed, 100f, maxspeed);
    ...
    private void SoundsInitialize()
    {
        ...
        if (useNOS)
        {
            NOSSound = RCC_CreateAudioSource.NewAudioSource(gameObject, "NOS Sound AudioSource", 1000, 2000, 1f, NOSClip, true, false, false);
        }
    ...
    private void DamageInit()
    {
        ...
        if (contactSparkle)
        {
            ...
            {
                GameObject sparks = Instantiate(contactSparkle, transform.position, Quaternion.identity) as GameObject;
    ...
    private void Update()
    {
        ...
        //   print("Nos :" +NoS + "boostInput" + _boostInput + "_gasInput" + _gasInput +"Turbo :"+turboBoost);
        if
            (RearLeftWheelCollider.wheelCollider.isGrounded == false)
        {
            Carshadow.SetActive(false);
            showshadow = true;
        }
        else if (showshadow == true && RearLeftWheelCollider.wheelCollider.isGrounded == true)
        {
            Carshadow.SetActive(true);
        }
        //if (camreverser)
        //{
        //    Bostshashka.gameObject.transform.GetChild(0).gameObject.SetActive(false);
    ...
    private void Engine()
    {
        ...
        //Speed.
        speed = rigid.velocity.magnitude * 3.6f * speedMult;
    ...
    public bool groundedRL, groundedFL, groundedRR, groundedFR;
    ...
    private void AntiRollBars()
    {
        ...
        groundedFL = FrontLeftWheelCollider.wheelCollider.GetGroundHit(out FrontWheelHit);
        ...
        groundedFR = FrontRightWheelCollider.wheelCollider.GetGroundHit(out FrontWheelHit);
        ...
        groundedRL = RearLeftWheelCollider.wheelCollider.GetGroundHit(out RearWheelHit);
        ...
        groundedRR = RearRightWheelCollider.wheelCollider.GetGroundHit(out RearWheelHit);
    ...
    {
        if (groundedRL && groundedFL && groundedRR && groundedFR)
            return true;
        else
            return false;
    ...
    //public void AutoNos()
    //{
    //    if (GamePlayManager.inst.N)
    //    {
    //        GamePlayManager.inst.N = false;
    //        _boostInput = 0f;
    //        NoS = 0;
    //        camreverser = true;
    //        GamePlayManager.inst.Nos_Button.gameObject.GetComponent<Button>().interactable = true;
    //        GamePlayManager.inst.Nos_Button2.gameObject.GetComponent<Button>().interactable = true;
    //        GamePlayManager.inst.NosHover.SetActive(true);
    ...
    public void NOS()
    {
        ...
        if (nos_IsActive)
        {
            SFX.set_statusAirEffect(true, isGroundedCheck());
        ...
        {
            SFX.set_statusAirEffect(false, isGroundedCheck());
            if (NOSSound.isPlaying)
            {
                NOSSound.Stop();
    ...
    public void SetTorqueCurves()
    {
        ...
        {
            maxSpeedForGear[j] = Mathf.Lerp(0f, maxspeed * 1.1f, (j + 1) / (float)(totalGears));
        }
        ...
        if (autoGenerateTargetSpeedsForChangingGear)
        {
            ...
            {
                targetSpeedForGear[k] = Mathf.Lerp(0, maxspeed * Mathf.Lerp(0f, 1f, gearShiftingThreshold), ((k + 1) / (float)(totalGears)));
            }
        }
            ...
            if (orgMaxSpeed != maxspeed || orgGearShiftingThreshold != gearShiftingThreshold)
            {
                ...
                for (int i = 0; i < totalGears; i++)
                {
                    ...
                    if (i != 0)
                    {
                        ...
                        engineTorqueCurve[i].MoveKey(0, new Keyframe(0, Mathf.Lerp(1f, .05f, (i + 1) / (float)totalGears)));
                        engineTorqueCurve[i].AddKey(Mathf.Lerp(0, maxspeed * .5f, (i / (float)(totalGears))), Mathf.Lerp(1f, .5f, (i / (float)(totalGears))));
                        engineTorqueCurve[i].AddKey(Mathf.Lerp(0, maxspeed * 1f, ((i + 1) / (float)(totalGears))), .15f);
                        engineTorqueCurve[i].AddKey(Mathf.Lerp(0, maxspeed, ((i + 1) / (float)(totalGears))) * 2f, -3f);
